...
function pageFor(route){return {dashboard:"index.html",add:"add.html",transfer:"transfer.html",history:"history.html",savings:"savings.html",tutorial:"tutorial.html",profile:"profile.html",backup:"backup.html",admin:"admin.html"}[route]||"index.html"}
...
if(state.session&&user()&&user().approved){shell();const map={dashboard:renderDashboard,add:renderAdd,transfer:renderTransfer,history:renderHistory,savings:renderSavings,tutorial:renderTutorial,profile:renderProfile,backup:renderBackup,admin:renderAdmin};if(currentRoute==="admin"&&user().role!=="admin"){window.location.href="index.html"}else{(map[currentRoute]||renderDashboard)()}}else{state.session=null;sessionStorage.removeItem(SESSION_KEY);applyPreferences(defaultPreferences());if(currentRoute!=="dashboard"){window.location.href="index.html"}else renderAuth()}
})();
